#代06-例4.2-基于状态值函数的随机环境扫地机器人任务策略评估
import numpy as np
    ...
    while True:
        ...
        for s in suqe:  # 遍历所有状态 [0~25]
            ...
            for a in action:
                ...
                if next_state == 12:
                    v += policy[s][newAction] * (0.8 * (rewards + gamma * V[s]) + 0.15 * (gamma * V[s]) + 0.05 * (
                                env_rewards + gamma * V[env_state]))
                    print(" %.2f*(0.8*（%d+%.1f*%.3f)+0.15*（%.f）+0.05*(%.2f+%.2f))+" % (
                        policy[s][newAction], rewards, gamma, V[s], V[s], env_rewards, V[env_state]), end="")
                else:
                    v += policy[s][newAction] * (
                                0.8 * (rewards + gamma * V[next_state]) + 0.15 * (gamma * V[s]) + 0.05 * (
                                    env_rewards + gamma * V[env_state]))
                    print(" %.2f*(0.8*（%d+%.1f*%.3f)+0.15*（%.f）+0.05*(%.2f+%.2f))+" % (
